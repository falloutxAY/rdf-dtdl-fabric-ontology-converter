...
class DTDLStreamAdapter:
    ...
    def convert_streaming(
        ...
        if not stream_result.success:
            raise ValueError(f"Streaming failed: {stream_result.error_message}")
        
        # Convert collected interfaces using DTDLConverter
        try:
        ...
        # Parse collected interface dicts into DTDLInterface objects
        parser = DTDLParser()
        
        # Create mock parse result with pre-loaded interfaces
        from dataclasses import dataclass, field
        ...
        # Parse each interface dict
        parsed_interfaces = []
        if stream_result.data:
            for interface_dict in stream_result.data.interfaces:
                try:
                    parsed = parser._parse_interface(interface_dict, "streaming")
                    if parsed:
                        parsed_interfaces.append(parsed)
                except Exception as e:
                    logger.warning(f"Failed to parse interface: {e}")
        
        # Convert to Fabric format
        converter = DTDLConverter(
